...
module Kafka
  module Sasl
    class OAuthBearer
      ...
      attr_reader :client_id, :client_secret, :server_url, :token_url, :scopes

      def initialize(logger:, client_id:, client_secret:, server_url:, token_url:, scopes:)
        @logger = logger
        @client_id = client_id
      ...
      def authenticate!(host, encoder, decoder)
        ...
        token = construct_token(access_token)
      ...
      def get_access_token
        ...
        client = OAuth2::Client.new(client_id, client_secret, site: server_url, token_url: token_url, auth_scheme: :basic_auth)
        response = client.client_credentials.get_token(scope: Array(@scopes).join(' '))
        response.token
      end

      def construct_token(access_token)
        token_ext = [1].pack("C")
